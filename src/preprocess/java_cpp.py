...
def generate_pairs(source_data_dir, target_data_dir):
    ...
    :param source_data_dir: 源数据目录，包含多个问题的解决方案（Java 和 C++）。
    :param target_data_dir: 目标数据存储目录。
    """

    def read_code_java(path):
        try:
            with open(path, 'r', encoding='utf-8') as f:
                text = []
                for line in f.readlines():
                    if any([line.strip().startswith('package'), line.strip().startswith('import')]):
                        continue
                    text.append(line)
                text = ''.join(text)
            return text
    ...
    for problem_dir in tqdm(os.listdir(source_data_dir), desc="Processing problem directories"):
        ...
        java_dir = os.path.join(problem_path, "Java")
        cpp_dir = os.path.join(problem_path, "C++")
        if not (os.path.exists(java_dir) and os.path.exists(cpp_dir)):
            continue

        # 处理 Java 文件夹
        if os.path.exists(java_dir):
            for java_file in os.listdir(java_dir):
                code = read_code_java(os.path.join(java_dir, java_file))
                if code and code not in unique_codes:
                    total_code_categories.append((code, problem_dir))
                    unique_codes.add(code)
                    code_language_map[code] = 'Java'

        # 处理 C++ 文件夹
        if os.path.exists(cpp_dir):
            for cpp_file in os.listdir(cpp_dir):
                code = read_code_cpp(os.path.join(cpp_dir, cpp_file))
                if code and code not in unique_codes:
                    total_code_categories.append((code, problem_dir))
                    unique_codes.add(code)
